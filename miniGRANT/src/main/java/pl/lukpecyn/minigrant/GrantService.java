...
public class GrantService {
	...
	public int addGrant(Grant grant) {
		String sql = "INSERT INTO grants(donor_id,beneficiary_id,name,date_begin,date_end,status) VALUES(?,?,?,?,?,?)";
		return jdbcTemplate.update(sql,grant.getDonor().getId(),grant.getBeneficiary().getId(),grant.getName(),grant.getDateBegin(),grant.getDateEnd(),grant.getStatus());
	}
	
	public int updateGrant(Grant grant) {
		String sql = "UPDATE grants SET donor_id=?, beneficiary_id=?, name=?,date_begin=?,date_end=?,status=? WHERE id=?";
		return jdbcTemplate.update(sql,grant.getDonor().getId(),grant.getBeneficiary().getId(),grant.getName(),grant.getDateBegin(),grant.getDateEnd(),grant.getStatus(),grant.getId());
	}
